const app = {
    state: {
        ...
        vendors: [
            ...
            {
                id: 5, name: "4Graph", category: "Graphics", country: "Italy", city: "Caserta", website: "https://4graph.it", phone: "+39 0823 123456", email: "info@4graph.it", speciality: "Paper based, Brochures",
                pricing: []
            },
            // Additional Vendors from 'Study'
            { id: 6, name: "Ovation", category: "Venue Services", country: "UAE", city: "Dubai", website: "", phone: "", email: "", speciality: "DMC, Hostess", pricing: [] },
            { id: 7, name: "Krystoff", category: "Construction", country: "Germany", city: "Nuremberg", website: "", phone: "", email: "", speciality: "Woodwork", pricing: [] },
    ...
    renderProjects: () => {
        const content = document.getElementById('content-area');
        // Filter for 2026 projects only for the main view to avoid cluttering with historical data
        const projects = app.state.projects
            .filter(p => new Date(p.setupDate).getFullYear() >= 2026)
            .sort((a, b) => new Date(a.setupDate) - new Date(b.setupDate));

        content.innerHTML = `
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom: 24px;">
                <h2>${app.t('projects')}</h2>
                <button class="btn btn-primary" onclick="app.openProjectModal()">
                    <i data-lucide="plus-circle" style="width:16px;"></i> ${app.t('new_project')}
                </button>
            </div>
                    ...
                    <table style="width:100%; border-collapse:collapse;">
                        <thead>
                            <tr style="background:rgba(255,255,255,0.02); border-bottom:1px solid var(--glass-border);">
                                <th style="text-align:left; padding:16px; color:var(--text-secondary);">Project / Event</th>
                                <th style="text-align:left; padding:16px; color:var(--text-secondary);">Setup Date</th>
                                <th style="text-align:left; padding:16px; color:var(--text-secondary);">PM</th>
                                <th style="text-align:left; padding:16px; color:var(--text-secondary);">Status</th>
                                <th style="text-align:left; padding:16px; color:var(--text-secondary);">Provider</th>
                                <th style="text-align:right; padding:16px; color:var(--text-secondary);">Actions</th>
                            </tr>
                        </thead>
                        <tbody>
                            ${projects.length > 0 ? projects.map(p => `
                                <tr style="border-bottom:1px solid rgba(255,255,255,0.02); transition:background 0.2s;" onmouseover="this.style.background='rgba(255,255,255,0.03)'" onmouseout="this.style.background='transparent'">
                                    <td style="padding:16px;">
                                        <div style="font-weight:bold; color:white;">${p.client}</div>
                                        <div style="font-size:0.85rem; color:var(--text-secondary);">${p.event} <span style="opacity:0.6;">(${p.sqm}m²)</span></div>
                                    </td>
                                    <td style="padding:16px; color:#e0e0e0;">${app.formatDate(p.setupDate)}</td>
                                    <td style="padding:16px;">
                                        <div style="display:flex; align-items:center; gap:8px;">
                                            <div style="width:24px; height:24px; border-radius:50%; background:${p.pm === 'Amir' ? '#3b82f6' : p.pm === 'Gema' ? '#10b981' : p.pm === 'Elena' ? '#f59e0b' : '#8b5cf6'}; display:flex; align-items:center; justify-content:center; font-size:0.7rem; font-weight:bold; color:white;">
                                                ${p.pm.charAt(0)}
                                            </div>
                                            <span>${p.pm}</span>
                                        </div>
                                    ...
                                    <td style="padding:16px; color:var(--text-secondary);">${p.provider || '-'}</td>
                                    <td style="padding:16px; text-align:right;">
                                        <button class="btn btn-outline btn-sm" onclick="app.editProject(${p.id})" style="padding:4px 8px;">
                                            <i data-lucide="edit-2" style="width:14px;"></i>
                                        </button>
                                        <button class="btn btn-outline btn-sm" onclick="app.deleteProject(${p.id})" style="padding:4px 8px; color:var(--danger); border-color:rgba(239,68,68,0.3);">
                                            <i data-lucide="trash-2" style="width:14px;"></i>
                                        </button>
                                    </td>
                                </tr>
                            `).join('') : '<tr><td colspan="6" style="text-align:center; padding:40px; color:var(--text-secondary);">No active projects for 2026. Check the Archive.</td></tr>'}
                        </tbody>
                    </table>
                </div>
            </div>
    ...
    renderTradeshows: (yearFilter) => {
        ...
        // Apply Filters
        const filters = app.state.tradeshowFilters || { country: '', venue: '' };
        ...
        const filteredShows = shows.filter(s => {
            ...
            return matchYear && matchCountry && matchVenue;
        });
    ...
    setTradeshowFilter: (type, val) => {
        if (!app.state.tradeshowFilters) app.state.tradeshowFilters = { country: '', venue: '' };

        if (type === 'reset') {
            app.state.tradeshowFilters = { country: '', venue: '' };
        } else {
    ...
    renderClients: () => {
        ...
        app.state.projects.forEach(p => {
            ...
            if (!clientData[name]) clientData[name] = new Set();
            const show = p.tradeshow || (p.event.includes('@') ? p.event.split('@')[1] : p.event) || '';
            if (show) clientData[name].add(show.trim().toUpperCase());
        });
            ...
            <div style="margin-bottom:24px;">
                ...
                <div style="display:grid; grid-template-columns: repeat(auto-fill, minmax(300px, 1fr)); gap:16px;">
                    ${sortedClients.length > 0 ? sortedClients.map(c => `
                        <div class="card" style="border-top:3px solid var(--accent); transition: transform 0.2s;" onmouseover="this.style.transform='translateY(-2px)'" onmouseout="this.style.transform='translateY(0)'">
                            <h3>${c}</h3>
                            <div style="margin-top:12px;">
                                <div style="font-size:0.75rem; color:var(--text-secondary); margin-bottom:8px; text-transform:uppercase; letter-spacing:0.5px;">History:</div>
    ...
    renderProjects: () => {
        ...
        const filterPM = app.state.filterPM || '';
        ...
        let filteredProjects = app.state.projects.filter(p => {
            ...
            const matchesPM = filterPM === '' || p.pm === filterPM;
            return matchesClient && matchesEvent && matchesPM;
        });
                    ...
                    <table style="width:100%; border-collapse:collapse;">
                        <thead>
                            <tr style="background:rgba(255,255,255,0.05); text-align:left;">
                                ...
                                <th style="padding:16px;">Show Start</th>
                                <th style="padding:16px;">Setup Date</th>
                                <th style="padding:16px;">PM</th>
    ...
    sortProjects: (field) => {
        if (app.state.projectSortField === field) {
            // Toggle
            app.state.projectSortDir = app.state.projectSortDir === 'asc' ? 'desc' : 'asc';
        } else {
            app.state.projectSortField = field;
            app.state.projectSortDir = 'asc';
        }
        app.renderProjects();
    },
    ...
    renderVendors: () => {
        ...
        content.innerHTML = `
             <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom: 24px;">
                ...
                <div style="display:flex; gap:12px;">
                    <div style="position:relative;">
                        <input type="text" placeholder="Search vendors (name, country, specialty)..." value="${app.state.vendorFilter || ''}" 
            ...
            <div style="display:grid; grid-template-columns: repeat(auto-fill, minmax(400px, 1fr)); gap:20px;">
                ${filteredVendors.map(v => `
                    <div class="card" style="position:relative; transition:transform 0.2s; hover:transform:translateY(-2px); border-top: 4px solid var(--accent);">
                         ...
                         ${v.pricing && v.pricing.length > 0 ? `
                            <div style="margin-bottom:16px;">
                                <div style="font-size:0.8rem; font-weight:bold; margin-bottom:8px; color:var(--accent); display:flex; justify-content:space-between; align-items:center;">
                                    ...
                                    <span style="font-size:0.7rem; color:var(--text-secondary);">Updated 2025</span>
                                </div>
                        ...
                        <div style="display:flex; gap:8px; flex-wrap:wrap; margin-top:auto;">
                             ${v.website ? `
                                <a href="${v.website}" target="_blank" class="btn btn-outline btn-sm" style="color:var(--accent); border-color:var(--accent);">
                                    <i data-lucide="globe" style="width:12px;"></i> Website
                                </a>
                             ` : ''}
                             ${v.email ? `
                                <a href="mailto:${v.email}" class="btn btn-outline btn-sm">
                                    <i data-lucide="mail" style="width:12px;"></i> Contact
                                </a>
                             ` : ''}
                             ${v.phone ? `
                                <a href="tel:${v.phone}" class="btn btn-outline btn-sm">
                                    <i data-lucide="phone" style="width:12px;"></i> Call
                                </a>
                             ` : ''}
                        </div>
                    </div>
